...
const Home = () => {
  return (
    <>
    <Navbar />
    <main className="min-h-screen bg-night-bg text-night-text px-4 py-20 flex flex-col items-center">
      {/* Hero Section */}
      <motion.section
      ...
      >
        <h1 className="text-4xl sm:text-5xl font-serif text-night-heading mb-6">
          Unlock AI Decisions with Transparency
        </h1>
        <p className="text-lg sm:text-xl text-night-muted mb-8 font-sans">
        ...
        <Link
          to="/signup"
          className="inline-block px-6 py-3 rounded-lg bg-button-gradient text-white font-semibold text-lg hover:opacity-90 transition"
        >
          Get Started
        </Link>
      </motion.section>
      ...
      <motion.section
        ...
        className="mt-24 grid grid-cols-1 sm:grid-cols-3 gap-8 max-w-5xl w-full"
      >
        {[
          {
          ...
          >
            <h3 className="text-xl font-serif text-night-heading mb-2">{feature.title}</h3>
            <p className="text-sm text-night-muted">{feature.desc}</p>
          </div>
        ))}
      </motion.section>
    </main>
